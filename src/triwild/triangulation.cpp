...
bool triwild::triangulation::load_input(
    const std::string &input, Eigen::MatrixXd &V,
  ...
  if (V.rows() == 0)
    ...
    for (int i = 0; i < edges.size(); i++) {
      if (edges[i][0] < 0 || edges[i][0] >= V.rows() || edges[i][1] < 0 ||
          edges[i][1] >= V.rows())
        return false;
    }
  }
...
void triwild::triangulation::preprocessing(
    Eigen::MatrixXd &V, std::vector<std::array<int, 2>> &edges,
    GEO::Mesh &b_mesh) {
  auto build_b_mesh = [](const Eigen::MatrixXd &V,
  ...
  auto set_global_parameters = [&]() {
    ...
    if (args.target_edge_len == -1)
      args.target_edge_len =
          args.diagonal_len *
          args.edge_length_r; // args.i_edge_length == 20 in init
    ...
    if (!args.is_preserving_feature)
      ...
      args.min_edge_length =
          (args.diagonal_len * 1e-4 * 2) / args.target_edge_len;
  ...
  if (args.is_preserving_feature) {
    ...
    for (int i = 0; i < feature::features.size(); i++) {
      if (!feature::features[i]->v_ids.empty()) {
        for (int j = 0; j < feature::features[i]->v_ids.size(); j++) {
          feature::features[i]->v_ids[j] = VI(feature::features[i]->v_ids[j]);
          if (j > 0 && feature::features[i]->v_ids[j] ==
                           feature::features[i]->v_ids[j - 1]) {
            feature::features[i]->v_ids.erase(
                feature::features[i]->v_ids.begin() + j);
            feature::features[i]->paras.erase(
                feature::features[i]->paras.begin() + j);
            j--;
            continue;
          }
        }
    ...
    cout << "#v " << old_cnt_v << "->" << V.rows() << endl;
    cout << "#e " << old_cnt_e << "->" << edges.size() << endl;
    cout << "#secondary_features = " << feature::secondary_features.size()
         << endl;
  } else {
    set_global_parameters();
    build_b_mesh(V, edges, b_mesh);
...
void triwild::triangulation::simplify_input(
  ...
  //        for (int j = 0; j < feature::secondary_features[i]->v_ids.size();
  //        j++)
  //            tag_secondary_feature_vs[feature::secondary_features[i]->v_ids[j]]
  //            = i;
  ...
  for (int i = 0; i < feature::secondary_features.size(); i++) {
    for (int j = 0; j < feature::secondary_features[i]->v_ids.size(); j++)
      tag_secondary_feature_vs[feature::secondary_features[i]->v_ids[j]].insert(
          i);
    //        is_freezed[feature::secondary_features[i]->v_ids.front()] = true;
    //        is_freezed[feature::secondary_features[i]->v_ids.back()] = true;
  }
  ...
  std::priority_queue<ElementInQueue_s, std::vector<ElementInQueue_s>, cmp_s>
      queue_s;
  for (auto &e : edges) {
  ...
  auto update_secondary_feature = [&](int v1_id, int v2_id) -> void {
    ...
    for (int feature_id : tmp_tag_secondary_feature_vs_v1_id) {
      ...
      if ((j + 1 < v_ids.size() && v_ids[j + 1] == v2_id) ||
          (j - 1 >= 0 && v_ids[j - 1] == v2_id)) {
    ...
    //        feature::secondary_features.size(); feature_id++) {
  ...
  auto check = [&]() -> void {
    ...
    int old_edge_size =
        std::count(e_is_removed.begin(), e_is_removed.end(), false);
  ...
  while (!queue_s.empty()) {
    ...
    if (!is_loop) {
      ...
      if (is_clean_vertex(edges[changed_e_id][1]))
        queue_s.push(ElementInQueue_s(
            {{edges[changed_e_id][1], edges[changed_e_id][0]}}, new_l));
    } else {
      for (int e_id : conn_es[v1_id]) {
        e_is_removed[e_id] = true;
        if (edges[e_id][0] == v2_id || edges[e_id][1] == v2_id)
  ...
  for (int i = 0; i < feature::features.size(); i++) {
    for (int j = 0; j < feature::features[i]->v_ids.size(); j++)
      feature::features[i]->v_ids[j] =
          map_v_ids[feature::features[i]->v_ids[j]];
  }
  for (int i = 0; i < feature::secondary_features.size(); i++) {
    if (feature::secondary_features[i]->v_ids.size() < 2) {
      feature::secondary_features.erase(feature::secondary_features.begin() +
                                        i);
      i--;
      continue;
    }
    for (int j = 0; j < feature::secondary_features[i]->v_ids.size(); j++) {
      ...
      //            {
      //                feature::secondary_features[i]->v_ids.erase(feature::secondary_features[i]->v_ids.begin()
      //                + j); j--; continue;
      //            }
      assert(v_is_removed[feature::secondary_features[i]->v_ids[j]] == false);
      feature::secondary_features[i]->v_ids[j] =
          map_v_ids[feature::secondary_features[i]->v_ids[j]];
    }
    //        if (feature::secondary_features[i]->v_ids.size() < 2) {
    //            feature::secondary_features.erase(feature::secondary_features.begin()
    //            + i); i--; continue;
...
void triwild::triangulation::BSP_subdivision(
    const Eigen::MatrixXd &V, const std::vector<std::array<int, 2>> &edges,
  ...
  Eigen::RowVector2d min(args.box_min(0) - args.target_edge_len,
                         args.box_min(1) - args.target_edge_len);
  Eigen::RowVector2d max(args.box_max(0) + args.target_edge_len,
  ...
  for (double i = 0; i <= nx; i++) {
    for (double j = 0; j <= ny; j++) {
      Eigen::RowVector2d p(i / nx * min[0] + (1 - i / nx) * max[0],
                           j / ny * min[1] + (1 - j / ny) * max[1]);
      GEO::vec3 p0(p[0], p[1], 0);
      GEO::vec3 nearest_point;
      double sq_dist;
      GEO::index_t prev_facet =
          b_tree.nearest_facet(p0, nearest_point, sq_dist);
  ...
  for (int i = 0; i < delaunay2d->nb_cells(); i++)
    bsp_faces[i] = {
        {cell_adj[i * 3], cell_adj[i * 3 + 1], cell_adj[i * 3 + 2]}};
  ...
  for (int i = 0; i < new_V.rows(); i++) {
    ...
    if ((bsp_vertices[i].pos[0] == min[0] ||
         bsp_vertices[i].pos[0] == max[0]) &&
  ...
  //        dV.row(i) << bsp_vertices[i].pos[0].to_double(),
  //        bsp_vertices[i].pos[1].to_double(), 0;
  //    for (int i = 0; i < bsp_faces.size(); i++)
  ...
  for (int e_id = 0; e_id < edges.size(); e_id++) {
    ...
    std::vector<int> tmp =
        optimization::set_intersection(conn_fs[v1_id], conn_fs[v2_id]);
    ...
    while (true) {
      ...
      for (int f_id : tmp_conn_fs) {
        ...
        int r_v_id =
            std::find(bsp_faces[f_id].begin(), bsp_faces[f_id].end(), v_id) -
        ...
        for (int i = 1; i < n_size - 1; i++) {
          ...
          if (!segment_intersection(mesh, v_id, v2_id, p1_id, p2_id,
                                    is_cross_p1, is_cross_p2, intersection_v))
            continue;
          ...
          if (is_cross_p1) {
            ...
            std::vector<int> n_f_ids =
                optimization::set_intersection(conn_fs[v_id], conn_fs[p1_id]);
            for (int n_f_id : n_f_ids)
              tmp_conn_fs.erase(n_f_id);
            if (i != 1) {
              int new_f_id = split_a_face(r_v_id, i, n_size, f_id);
              //                            std::vector<int> n_f_ids =
              //                            optimization::set_intersection(conn_fs[v_id],
              //                            conn_fs[p1_id]);
            ...
            //                        tmp_conn_fs.erase(f_id);
            std::vector<int> n_f_ids =
                optimization::set_intersection(conn_fs[v_id], conn_fs[p2_id]);
            for (int n_f_id : n_f_ids)
              tmp_conn_fs.erase(n_f_id);
            if (i != n_size - 2) {
            ...
            std::vector<int> p1p2_e_ids = optimization::set_intersection(
                tag_boundary_es[p1_id], tag_boundary_es[p2_id]);
            ...
            std::vector<int> n_f_ids =
                optimization::set_intersection(conn_fs[p1_id], conn_fs[p2_id]);
            ...
            for (int k = 0; k < bsp_faces[n_f_id].size(); k++) {
              if (bsp_faces[n_f_id][k] == p2_id &&
                  bsp_faces[n_f_id][(k + 1) % bsp_faces[n_f_id].size()] ==
                      p1_id) {
                bsp_faces[n_f_id].insert(bsp_faces[n_f_id].begin() + k + 1,
                                         new_v_id);
                break;
              }
            }
  ...
  for (int f_id = 0; f_id < bsp_faces.size(); f_id++) {
    ...
    if (bsp_faces[f_id].size() == 3) {
      mesh.tris.push_back(
          {{bsp_faces[f_id][0], bsp_faces[f_id][1], bsp_faces[f_id][2]}});
      continue;
    }
    for (int j = 1; j < int(bsp_faces[f_id].size()) - 1; j++) {
      mesh.tris.push_back(
          {{bsp_faces[f_id][0], bsp_faces[f_id][j], bsp_faces[f_id][j + 1]}});
    }
  }
  ...
  //        oV.row(i) << mesh.tri_vertices[i].posf[0],
  //        mesh.tri_vertices[i].posf[1], 0;
...
bool triwild::triangulation::segment_intersection(MeshData &mesh, int v_id,
                                                  int v2_id, int p1_id,
                                                  int p2_id, bool &is_cross_p1,
                                                  bool &is_cross_p2,
                                                  TriVertex &intersection_v,
                                                  bool is_check_bbox) {
  auto &bsp_vertices = mesh.tri_vertices;

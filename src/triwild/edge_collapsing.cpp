...
void triwild::optimization::edge_collapsing(MeshData& mesh, GEO::MeshFacetsAABB &b_tree) {
    ...
    if(mesh.is_curved) {
        ;//todo: clean up dirty nodes after simplification
    }
}
...
//                && !mesh.is_curved //todo: should be "skip checking feature edges"
                && l != 0
                && !is_valid_envelop(mesh, b_tree, v1_id, mesh.tri_vertices[v2_id].posf)) {
//                cout << "E";
                return false;
            }
    ...
    if(mesh.is_curved) {
        ...
        for (int t_id:curved_changed_t_ids) {
            ...
            if (tag_feature_e[0] < 0 && tag_feature_e[1] < 0 && tag_feature_e[2] < 0)
                continue;
            feature::get_new_nodes(tag_feature_e, {{mesh.tri_vertices[new_t[0]],
                                                           mesh.tri_vertices[new_t[1]],
                                                           mesh.tri_vertices[new_t[2]]}},
                                   new_nodes);
            if(new_nodes.empty())
                continue;

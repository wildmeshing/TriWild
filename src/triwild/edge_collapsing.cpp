                                             ...
                                             std::vector<std::array<int, 2>> &new_es)
{
    ...
    if (mesh.tri_vertices[v1_id].is_on_point)
    { //circle envelop
    ...
    if (mesh.is_curved)
    {
        ...
        for (int t_id : curved_changed_t_ids)
        {
            ...
            if (tag_feature_e[0] < 0 && tag_feature_e[1] < 0 && tag_feature_e[2] < 0)
                continue;
            feature::get_new_nodes(tag_feature_e, {{mesh.tri_vertices[new_t[0]], mesh.tri_vertices[new_t[1]], mesh.tri_vertices[new_t[2]]}},
                                   new_nodes);
            if (new_nodes.empty())
                continue;

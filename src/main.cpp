...
int main(int argc, char *argv[]) {
  ...
  app.add_option("--input", args.input, "Input segments in .obj format.");
  app.add_option("--output", args.output, "Output path.");
  app.add_option("--postfix", args.postfix,
                 "Add postfix into outputs' file name.");
  app.add_option("--feature-input", args.feature_input,
                 "Input feature json file.");
  //    app.add_option("--quality-output", args.quality_output,
  //    "Quality_output");
  app.add_option("--stop-quality", args.stop_quality,
                 "Specify max AMIPS energy for stopping mesh optimization.");
  app.add_option("--max-its", args.max_its,
                 "Max number of mesh optimization iterations.");
  app.add_option("--stage", args.stage, "Specify envelope stage");
  app.add_option("--envelope-r", args.epsilon,
                 "relative envelope epsilon_r. Absolute epsilonn = epsilon_r * "
                 "diagonal_of_bbox");
  app.add_option(
      "--feature-envelope-r", args.feature_epsilon,
      "Relative feature envelope mu_r. Absolute mu = mu_r * diagonal_of_bbox");
  app.add_option(
      "--target-edge-length", args.target_edge_len,
      "Absolute target edge length l."); // absolute value fore edge length
  app.add_option(
      "--target-edge-length-r", args.edge_length_r,
      "Relative target edge length l_r. Absolute l = l_r * "
      "diagonal_of_bbox"); // relative value for edge length
                           //     app.add_option("--dd", args.i_dd, "");
  app.add_option("--log-file", args.log_file, "Output a log file.");
  app.add_option("--min-angle", args.flat_feature_angle,
                 "Desired minimal angle.");

  app.add_flag("--mute-log", args.mute_log, "Mute prints.");
  //    app.add_flag("--enable-debug-mesh", args.enable_debug_mesh, "Enable
  //    output debug mesh.");
  ...
  //    app.add_flag("--diffusion-curves", diffusion_curves, "Export for
  //    diffusion curves");
  ...
  app.add_option("--cut-holes", hole_file,
                 "Input a .xyz file for specifying points inside holes you "
                 "want to remove.");

  app.add_flag("--output-linear-mesh", args.output_linear,
               "Output linear mesh for curved pipeline.");
  ...
  if (args.output == "") {
    if (args.input == "")
      args.output = args.feature_input + "_" + args.postfix;
    else
  ...
  if (!skip_eps) {
    export_eps(mesh, line_width, line_col, point_size, point_col, f_line_width,
               f_line_col, f_point_size, f_point_col, s_f_line_width,
               s_f_line_col, s_f_point_size, s_f_point_col, draw_points,
               args.output + ".eps");
